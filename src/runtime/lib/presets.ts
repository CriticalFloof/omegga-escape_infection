...
export class PresetHandler {
    ...
    public static checkBrickadiaSaveFileIntegrity() {
        ...
        minigameQueries.forEach((fileName)=>{
            if(!fs.existsSync(`${brMinigamePath}/EI_${fileName}.bp`)){
                unavailibleFiles.push({destinationName:`EI_${fileName}.bp`, sourceName: `${fileName}.bp`, destinationPath:brMinigamePath, sourcePath: `${pluginGamemodePath}/${fileName}`})
            }
        })
        environmentQueries.forEach((fileName)=>{
            if(!fs.existsSync(`${brEnvironmentPath}/EI_${fileName}_environment.bp`)){
                unavailibleFiles.push({destinationName:`EI_${fileName}_environment.bp`, sourceName: `${fileName}_environment.bp`, destinationPath:brEnvironmentPath, sourcePath: `${pluginMapsPath}/${fileName}`})
            }
        })
        compiledMapQueries.forEach((fileName)=>{
            if(!fs.existsSync(`${brBuildCompiledPath}/${fileName}.brs`)){
                unavailibleFiles.push({destinationName:`${fileName}.brs`, sourceName: `${fileName}.brs`, destinationPath:brBuildCompiledPath, sourcePath: `${pluginMapsPath}/${fileName}`})
            }
        })
        sourceMapQueries.forEach((fileName)=>{
            if(!fs.existsSync(`${brBuildSourcePath}/${fileName}_source.brs`)){
                unavailibleFiles.push({destinationName:`${fileName}_source.brs`, sourceName: `${fileName}_source.brs`, destinationPath:brBuildSourcePath, sourcePath: `${pluginMapsPath}/${fileName}/source`})
            }
        })
    ...
    public static getEditableMaps() {
        const playableMaps = this.getAvailibleMaps()

        const brSavePath = Runtime.omegga.savePath
        const brBuildSourcePath = `${brSavePath}/Escape_Infection/Source`

        let readyMaps = []
        for (let i = 0; i < playableMaps.length; i++) {
            const mapName = playableMaps[i]
            
            if(!fs.existsSync(`${brBuildSourcePath}/${mapName}_source.brs`)) continue;
            // File has all nessesary files present and therefore can be listed as an editable map to pick from. yay!
            readyMaps.push(mapName)
        }
        return readyMaps
    }
